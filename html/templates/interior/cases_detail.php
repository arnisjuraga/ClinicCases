<div class="case_detail_bar">
    ...
    <div class="assigned_people">
        ...
        <ul>
            ...
            <li class="slide closed">
                <a title="Click here to see history (all users ever on this case)">Assigned:</a>
            </li>


            <?php if ($assigned_users_data) {
                foreach ($assigned_users_data as $user) {
                    $thumbnail = thumbify($user['picture_url']);

                    if ($user['user_case_status'] == "active") {
                        echo "<li class = 'active'><span><img class='thumbnail-mask' tabindex='1' id='imgid_" . $user['case_id'] . "_" . $user['username'] . "' src='$thumbnail' title='" . htmlspecialchars($user['first_name'],
                                ENT_QUOTES, 'UTF-8') . " " . htmlspecialchars($user['last_name'], ENT_QUOTES, 'UTF-8') . "'></span></li>";
                    } else {
                        echo "<li class = 'inactive'><span><img  class='thumbnail-mask' tabindex='1' id='imgid_" . $user['case_id'] . "_" . $user['username'] . "' src='$thumbnail' title='" . htmlspecialchars($user['first_name'],
                                ENT_QUOTES, 'UTF-8') . " " . htmlspecialchars($user['last_name'], ENT_QUOTES, 'UTF-8') . "'></span></li>";
                    }
...
<div class="case_detail_nav">
    ...
    <ul class="case_detail_nav_list">
        ...
        <li id="item7">Conflicts
            <span class="conflicts_number"></span>
        </li>
