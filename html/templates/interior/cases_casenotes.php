...
//tools are only called if this is a first request; otherwise we only need the new case notes data
...
TOOLS;
    echo $case_notes_data[0]['case_id'] ?? '-- neeksistējošs case --';
    echo <<<TOOLS
...
<div class="csenote csenote_new">
			...
			<div class="csenote_bar">
				...
				<label>Date:</label> <input type="hidden" name="csenote_date" class="csenote_date_value" 
				value="$this_date"> $selector
...
echo $data['twig_header'];

foreach ($case_notes_data as $case_notes) {

    $time = convert_case_time($case_notes['time']);
    echo "<div class='csenote mt-2' id='csenote_" . $case_notes['id'] . "'>
				<div class='csenote_bar'>
				<div class='csenote_bar_left hide_print'>
				<img class='thumbnail-mask' src='" . thumbify($case_notes['picture_url']) . "'>&nbsp " . username_to_fullname($dbh,$case_notes['username']) . "
				</div>
				<div class='csenote_bar_right'>
				<span class='csenote_date'>" . extract_date($case_notes['date']) . "</span> &#183; <span class='csenote_time'>" . $time[0] . $time[1] . "</span>";
