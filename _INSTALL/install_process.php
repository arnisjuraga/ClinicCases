...
if (!is_writable($_POST['doc_path']) || !is_dir($_POST['doc_path'])) {
    $resp = ["error"   => true,
             "message" => "<p class='config_error'>Sorry, it appears that the path to documents you specified (" . $_POST['doc_path'] . ") either does not exist or is not writable.  Please fix this and try again.</p>",
    ];
...
if (!is_writable($_POST['cc_path']) || !is_dir($_POST['cc_path'])) {
    $resp = ["error"   => true,
             "message" => "<p class='config_error'>Sorry, it appears that the path you specified (" . $_POST['cc_path'] . ") either does not exist or is not writable.  Please fix this and try again.</p>",
    ];
...
try {
    ...
    $resp = ['error' => true, 'message' => $message, 'html' => ''];
...
if ($error[1]) {
    ...
    $resp = ['error' => true, 'message' => 'There was an error adding data to your database.  Here is what the database said:' . $error_string];
    ...
    $resp = ['error' => false, 'message' => 'Installation successful', 'html' => $html];
