...
if (!$_SESSION['permissions']['activate_users'] == '1') {
    $resp = ['error' => true, 'message' => "You do not have permission to do this."];
...
if ($count < 1) {
    $resp = ['error' => true, 'message' => 'Error retrieving user.'];
    ...
    if ($error[1]) {
        $resp = ['error' => true, 'message' => "Sorry, there was an error resetting the password. "];
        echo json_encode($resp);
    } else {
        //Send email to user
        $user = $check['username'];
        ...
        $resp = ['error' => false, 'message' => "The user's password has been reset to <b>$gen_pass</b>"];
