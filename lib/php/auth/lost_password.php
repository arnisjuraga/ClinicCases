...
if ($count < 1) {
    $resp = ['error' => true, 'message' => 'There are no users with that email address on this server.  Please try again'];
    echo json_encode($resp);
} else {
    if ($count > 1) {
        $resp = [
            'error'   => true,
            'message' => 'There are multiple accounts associated with that email address.
    Please ask your administrator to reset your password.',
        ...
        if ($error[1]) {
            $resp = ['error' => true, 'message' => "Sorry, there was an error resetting your password.  Please contact your administrator."];
            ...
            $resp = ['error' => false, 'message' => "Your username and a new temporary password have been emailed to " . $email . ".  If it does not arrive in a few minutes, please check your spam folder."];
