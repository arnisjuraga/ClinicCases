...
if ($user_query->rowCount() < 1) {
    ...
    $json = ['login' => 'false', 'message' => '' . $msg . '', 'url' => 'null'];
...
} else //just set a session cookie with username to be used by timer
{
...
$json = ['login' => 'true', 'message' => 'Logging you in....', 'url' => '' . $target_url . ''];
$response = json_encode($json);
echo $response;
