...
switch ($type) {
    ...
    case 'new':
        ...
        if (!$_SESSION['permissions']['writes_journals'] == "1") {
            $response = ['error' => true, 'message' => 'Sorry, you do not have permission to add journals.'];
        ...
        if (!$error[1]) {
            ...
            $response = ['error' => false, 'newId' => $new_id];
    ...
    case 'edit':
        ...
        $data = ['text' => $text, 'id' => $id[0], 'reader' => $reader];
    ...
    case 'add_comment':
        ...
        $c = ['id' => $id[0], 'by' => $_SESSION['login'], 'text' => $comment_text, 'time' => $time];
...
if ($error[1]) {
    $return = ['error' => true, 'message' => 'Sorry, there was an error.'];

    echo json_encode($return);
} else {
    switch ($type) {
        case 'mark_read':
            $return = ['error' => false, 'message' => "Journals Marked Read."];
            echo json_encode($return);
            break;

        case 'mark_unread':
            $return = ['error' => false, 'message' => "Journals Marked Unread."];
            echo json_encode($return);
            break;

        case 'archive':
            $return = ['error' => false, 'message' => 'Journals Archived.'];
            echo json_encode($return);
            break;

        case 'edit':
            $return = ['error' => false, 'message' => 'Changes Saved.'];
            echo json_encode($return);
            break;

        case 'delete':
            $return = ['error' => false, 'message' => 'Journal Deleted.'];
            echo json_encode($return);
            break;

        case 'add_comment':
            $return = ['error' => false, 'message' => 'Comment Saved'];
            echo json_encode($return);
            break;
