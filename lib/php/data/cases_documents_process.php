...
if ($action == 'newfolder') {
    ...
    if (check_folder_unique($dbh, $container, $new_folder, $case_id) === true) {
        $return = ['message' => 'Sorry, that folder name is already in use.  Please choose another name.', 'error' => true];
...
if ($action == 'add_url') {
    ...
    $data = ['url_name' => $url_name, 'url' => $url, 'folder' => $path, 'user' => $username, 'case_id' => $case_id];
...
if ($action == 'new_ccd') {
    ...
    $allowed_editors = serialize([$username]);

    $data = ['ccd_name' => $ccd_name, 'local_file_name' => $local_file_name, 'folder' => $path, 'user' => $username, 'case_id' => $case_id, 'allowed_editors' => $allowed_editors];
...
if ($action == 'update_ccd') {
    ...
    $data = ['name' => $ccd_name, 'ccd_local_name' => $ccd_local_name, 'doc_id' => $ccd_id, 'ccd_text' => $ccd_text];
...
if ($action === 'change_ccd_permissions') {
    ...
    if ($ccd_lock === 'yes') {
        ...
        $allowed_editors = serialize(['all']);
    }

    $data = ['doc_id' => $ccd_id, 'perm' => $allowed_editors];
...
if ($action == 'cut') {
    if ($doc_type == 'folder') {
        ...
        if ($selection_path == '') {
            $return = ['message' => 'There was an error moving your folder. Please reload the page and try again', 'wait' => true, 'error' => true];
            echo json_encode($return);
            die;
        ...
        $data = ['target_path' => $target_path, 'new_selection_path' => $new_selection_path, 'item_id' => $item_id];
        ...
        // Bug Code:
        //$old_path = '' . "%";
        $data = ['old_path' => $old_path, 'case_id' => $case_id];
        ...
        foreach ($paths as $path) {
            ...
            $data = ['folder' => $new_subfolder_path, 'container' => $new_container];
        ...
        $data = ['target' => $target_path, 'item_id' => $item_id];
...
if ($action == 'copy') {
    if ($doc_type == 'folder') {
        ...
        $data = ['name' => $this_item['name'], 'extension' => $this_item['extension'], 'folder' => $target_path, 'username' => $username, 'case_id' => $case_id];
...
if ($error[1]) {
    $return = ['message' => 'Sorry,there was an error.', 'error' => true];
    echo json_encode($return);
} else {
    ...
    switch ($_POST['action']) {
        ...
        case "rename":
            if ($doc_type === 'folder') {
                $return = ['message' => 'Folder Renamed.', 'newPath' => $new_path];
            } else {
        ...
        case "add_url":
            $return = ['message' => 'Web address added.'];
            echo json_encode($return);
            break;

        case "new_ccd":
            $new_id = $dbh->lastInsertId();
            $return = ['ccd_id' => $new_id, 'ccd_title' => $ccd_name];
            echo json_encode($return);
            break;

        case "update_ccd":
            $return = ['message' => 'Changes saved', 'ccd_title' => $ccd_name];
            echo json_encode($return);
            break;
        ...
        case "open":
            if (isset($target_url)) {
                ...
                $return = [
                    'ccd_id'          => $ccd_id,
                    'ccd_title'       => $ccd_title,
                    'ccd_content'     => $ccd_content,
                    'ccd_permissions' => $ccd_permissions,
                    'ccd_owner'       => $ccd_owner,
                    'ccd_locked'      => $ccd_locked,
                ];
            }
            echo json_encode($return);
            break;
