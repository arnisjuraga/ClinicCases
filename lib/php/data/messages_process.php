...
function generate_recipients($dbh, $thread_id) //Get list of recipients for a reply
{
    ...
    $data = ['tos' => $parties['to'], 'ccs' => $parties['ccs'], 'from' => $parties['from']];
...
switch ($action) {
    ...
    case 'send':
        ...
        foreach ($new_tos as $to) {
            ...
            } elseif (stristr($to, '_spv_')) //user has selected a group that is defined by who the supervisor is
            {
            ...
            } elseif (stristr($to, '_all_on_case_')) //all users assigned to a particular case
            {
        ...
        if ($new_ccs) {
            ...
            foreach ($new_ccs as $cc) {
                ...
                } elseif (stristr($cc, '_spv_')) //user has selected a group that is defined by who the supervisor is
                {
        ...
        $data = ['tos' => $tos, 'sender' => $user, 'ccs' => $ccs, 'subject' => $new_subject, 'body' => $new_msg_text, 'assoc_case' => $assoc_case, 'sender_has_read' => $sender_has_read];
    ...
    case 'reply':
        ...
        $data = ['thread_id' => $thread_id, 'to' => $to, 'ccs' => $cc, 'sender' => $user, 'reply_text' => $reply_text, 'assoc_case' => $case];
    ...
    case 'forward':
        ...
        $data = ['forwards' => $forwards, 'id' => $thread_id];
        ...
        if (!$error[1]) {
            ...
            $data = ['thread_id' => $thread_id, 'to' => $to, 'ccs' => $cc, 'sender' => $user, 'forward_text' => $forward_text];
    ...
    case 'archive_all':
        ...
        if (!$error[1]) {
            ...
            foreach ($msgs as $msg) {
                ...
                $data = ['user' => $user_string, 'id' => $msg['id']];
...
if ($error[1]) {
    ...
    switch ($action) {
        ...
        case "unarchive":
            $return = ['message' => 'Message returned to Inbox.'];
            echo json_encode($return);
            break;
        ...
        case "mark_unread":
            $return = ['message' => 'Message marked unread.'];
            echo json_encode($return);
            break;

        case "archive_all":
            $return = ['message' => 'All inbox messages sent to archive.'];
            echo json_encode($return);
            break;
