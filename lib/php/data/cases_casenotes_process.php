...
switch ($_POST['query_type']) {
    case "add":
        ...
        $case_notes_process->execute([':case_id' => $case_id, ':date' => $date, ':time' => $time, ':description' => $description, ':user' => $user]);
        break;

    case "modify":
        $sql = "UPDATE cm_case_notes SET date = :date, time = :time, description = :description WHERE id = :case_note_id";
        $case_notes_process = $dbh->prepare($sql);
        $case_notes_process->execute([':date' => $date, ':time' => $time, ':description' => $description, ':case_note_id' => $case_note_id]);
        break;

    case "delete";
        $sql = "DELETE FROM cm_case_notes WHERE id = :case_note_id";
        $case_notes_process = $dbh->prepare($sql);
        $case_notes_process->execute([':case_note_id' => $case_note_id]);
        break;
}
...
if ($error[1]) {
    ...
    switch ($_POST['query_type']) {
        case "add":
            $return = ['message' => 'Case note added.'];
            echo json_encode($return);
            break;

        case "modify":
            $return = ['message' => 'Case note edited.'];
            echo json_encode($return);
            break;
