...
switch ($type) {
    case 'court':
        ...
        if ($_POST['court'][0] == '') {
            unset($_POST['court'][0]);
        } else //Array must start at index 1 for html purposes, so:
        {
...
if (count($result_errors) > 0) {
    ...
    switch ($type) {
        case 'court':
            $response = ['error' => false, "message" => 'Your courts have been updated.'];
            echo json_encode($response);
            break;

        case 'dispo':
            $response = ['error' => false, "message" => 'Your dispositions have been updated.'];
            echo json_encode($response);
            break;

        case 'referral':
            $response = ['error' => false, "message" => 'Your referrals have been updated.'];
            echo json_encode($response);
            break;

        case 'clinic':
            $response = ['error' => false, "message" => 'Your clinics have been updated.'];
            echo json_encode($response);
            break;

        case 'case':
            $response = ['error' => false, "message" => 'Your case types have been updated.'];
            echo json_encode($response);
            break;
    }
}
