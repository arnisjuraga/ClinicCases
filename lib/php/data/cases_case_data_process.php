...
foreach ($_POST as $key => $value) {
    ...
    if (substr($value, 0, 1) === "{" || substr($value, 0, 1 === "[")) //this is to stop php from turning integers (like id) into json
    {
...
switch ($action) {
        ...
        if (!$error[1]) {
            if (isset($_POST['adverse_parties'])) {
                ...
                foreach ($ap as $key => $a) {
                    ...
                    $data = ['case_id' => $_POST['id'], 'name' => $key];
    ...
    case 'delete':
        ...
        if (!empty($check_arr)) {
            ...
            $cols = $q->fetchAll();
            $col_array = [];
            $req_cols = ['id', 'clinic_id', 'first_name', 'middle_name', 'last_name', 'organization', 'date_open', 'date_close', 'opened_by', 'time_opened', 'time_closed'];
            foreach ($cols as $c) {
        ...
        if ($error[1]) {
            $return = ['message' => 'Sorry, there was an error deleting the case. Please try again.', 'error' => true];
            ...
            if ($error[1]) {
                $return = ['message' => 'Sorry, there was an error deleting associated case data. Some data may remain.', 'error' => true];
                echo json_encode($return);
            } else {
...
if ($error[1]) {
    ...
    switch ($action) {
        ...
        case 'update_new_case':
            ...
            $return = ["message" => htmlspecialchars($case_name, ENT_QUOTES, 'UTF-8') . " is now $text.", "error" => false];
            echo json_encode($return);
        ...
        case 'edit':
            ...
            $return = ["message" => htmlspecialchars($case_name, ENT_QUOTES, 'UTF-8') . " case $text.", "error" => false];
            echo json_encode($return);
        ...
        case 'delete':
            ...
            $return = ['message' => 'Case deleted.', 'error' => false];
            echo json_encode($return);
