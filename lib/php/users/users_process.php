...
switch ($action) {
    ...
    case 'create':
        ...
        if (!$error[1]) {
            ...
            $data = ['user' => $new_username, 'pass' => $pass, 'id' => $_POST['id']];
            $q->execute($data);
...
if ($error[1]) {
    $return = ['message' => 'Sorry, there was an error. Please try again.', 'error' => true];
    echo json_encode($return);
} else {
    switch ($action) {
        case 'activate':
            $return = ['message' => 'Users activated and notified by email.'];
            echo json_encode($return);
            break;
        ...
        case 'update':
            if ($_POST['new'] === 'yes' && $_POST['status'] === 'active') {
                $return = ['message' => 'User activated and notified by email.'];
            } else {
        ...
        case 'create':
            $return = ['message' => 'New account created.  The user has been notified by email'];
            echo json_encode($return);
            break;
