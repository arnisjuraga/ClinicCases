...
if (RECAPTCHA_PUBLIC_KEY !== '%recaptcha_public%' && RECAPTCHA_PUBLIC_KEY !== '(optional)') //Recaptcha is enabled
{
    ...
    if (!isValid()) {
        ...
        $response = ["error" => true, "message" => "ReCAPTCHA thinks you're a robot. Please check the box."];
...
if ($count > 0) {
    $response = ['error' => true, 'message' => 'There is already a user on this system with the email address ' . $_POST['email'] . '.  Perhaps you already have an account?'];
...
while (check_uniqueness($dbh, $proposed_username)) {
    if (is_numeric(substr($proposed_username, -1))) //we have already tried to make username unique by adding a number
    {
...
if ($error[1]) {
    $response = ['error' => true, 'message' => 'Sorry, there was an error.'];
    ...
    $response = [
        "error"   => false,
        "message" => "Account application successful",
