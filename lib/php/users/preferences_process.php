...
switch ($action) {
    ...
    case 'change_password':
        ...
        $data = ['id' => $_POST['id'], 'pword' => $pass];

        $q->execute($data);

        if ($q->rowCount() < 1) {
            $return = ['error' => true, 'message' => 'Your old password is wrong.'];
            ...
            $data = ['id' => $_POST['id'], 'new_pass' => $new_pass];
    ...
    case 'change_private_key':
        ...
        $data = ['private_key' => $new_key, 'id' => $_POST['id']];
...
if ($error[1]) {
    ...
    switch ($action) {
        case 'update_profile':
            $return = ['error' => false, 'message' => 'Your profile has been updated.'];
            echo json_encode($return);
            break;

        case 'change_password':
            $return = ['error' => false, 'message' => 'Your password has been changed.'];
            echo json_encode($return);
            break;
        ...
        case 'change_private_key':
            $return = ['error' => false, 'message' => 'Your private_key has been changed.'];
            echo json_encode($return);
            break;
    }
}
