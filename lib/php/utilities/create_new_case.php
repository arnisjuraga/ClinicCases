...
function create_new_case_number($dbh)
{
    ...
    if (stristr($m, 'Number')) {
        if (stristr($m, 'Infinite')) //our case number doesn't reset to zero at the beginning of each year
        {
...
if (!$_SESSION['permissions']['add_cases'] == "1") {
    $response = ['error' => true, 'message' => 'Sorry, you do not have permission to add cases.'];
    echo json_encode($response);
    die;
}

$q = $dbh->prepare("INSERT INTO `cm` (`id`, `clinic_id`,`organization`, `date_open`,`opened_by`
,`notes`
,`close_notes`
,`assigned_users`
) VALUES (NULL, ?, 'New Case', CURDATE(),?
, ''
, ''
, ''
);");
